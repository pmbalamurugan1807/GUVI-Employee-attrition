...
st.markdown("Analyze, visualize, and predict employee attrition using machine learning.")

# Sidebar Navigation 

st.sidebar.title("Navigation")
page = st.sidebar.radio("Go to:", ["Dataset Overview", "EDA and its Visualization", "Model Training & Evaluation", "Predict Attrition"])
...
#Model training & evaluation
    ...
    fig, ax = plt.subplots()
    sns.heatmap(cm, annot=True, fmt='d', cmap='Blues', xticklabels=['Stayed', 'Left'], yticklabels=['Stayed', 'Left'], ax=ax)
    st.pyplot(fig)
...
#Attrition
    ...
    if prob > 0.5:
        st.error("This employee is likely to leave. Consider proactive retention strategies.")
    else:
